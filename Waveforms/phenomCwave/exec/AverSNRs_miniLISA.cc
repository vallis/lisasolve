...
int main(){
   ...
   std::ofstream fout23("Data/eLISA_psd.dat");
   for (int i =0; i<n; i++){
   ...
   //std::ofstream fout("Data/MCevents_SNR_LISA.dat");
   //std::ofstream fout("Data/MCevent_SNReLISA.dat");
   std::ofstream fout("Data/MCevent_SNReLISA_comparHybr.dat");
   M = 2.e5;
   q = 1.0;
   m1 = 1.e5;
   chi = 0.0;
   ...
   double spin  = 0.6;
   chi = m1/M*spin + m2/M*spin;
   std::cout << "chi = " << chi << std::endl;
   std::cout << "tot mass = " << M << "   " << m1+m2 << std::endl;
   tc = year; 
   ...
   for (int i=0; i<nsources; i++){
     ...
     // std::cout << id_r << spr << id_s << spr << z << spr << M << spr << q << spr << phiS << spr \
      ...
      phiS = LISAWP_TWOPI * gsl_rng_uniform (r);   
      Cx = gsl_rng_uniform (r);   
      thetaS = acos(Cx);
      phiL = LISAWP_TWOPI * gsl_rng_uniform (r); 
      Cx = gsl_rng_uniform (r);   
      thetaL = acos(Cx);
      phi0 = LISAWP_TWOPI * gsl_rng_uniform (r); 
      ...
      std::cout << phiS << spr << thetaS << spr <<  phiL << spr << thetaL << spr <<  phi0 << H.iota << spr << H.psi << std::endl;
      //exit(0);
      ...
      std::cout << "SNR = " << sqrt(SNR2) << std::endl;
   ...
   /*   std::cout << "SNR^2 = " << SNR2 << std::endl;
      std::ofstream fout423("Data/TestAntoineHSNR.dat");
      for (int i=0; i<n; i++){
