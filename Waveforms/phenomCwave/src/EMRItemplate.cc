...
EMRItemplate::EMRItemplate(double Mass, double mu, double spin, double lambda){
...
EMRItemplate::EMRItemplate(const EMRItemplate& p){
    ...
    if(p.XAalpha.Size() != 0){
         XAalpha.ExpandEmpty(p.XAalpha.Size());
         XAalpha = p.XAalpha;
    }
    if(p.YEbeta.Size() != 0){
         YEbeta.ExpandEmpty(p.YEbeta.Size());
         YEbeta = p.YEbeta;
    }
    if(p.ZTgamma.Size() != 0){  
         ZTgamma.ExpandEmpty(p.ZTgamma.Size());
         ZTgamma = p.ZTgamma;
    }
...
EMRItemplate EMRItemplate::operator=(const EMRItemplate& p){
    ...
    if(p.XAalpha.Size() != 0){
      this->XAalpha.MakeEmpty();
      this->XAalpha.ExpandEmpty(p.XAalpha.Size());
      this->XAalpha = p.XAalpha;
    }
    if(p.YEbeta.Size() != 0){
      this->YEbeta.MakeEmpty();
      this->YEbeta.ExpandEmpty(p.YEbeta.Size());
      this->YEbeta = p.YEbeta;
    }
    if(p.ZTgamma.Size() != 0){  
      this->ZTgamma.MakeEmpty();
      this->ZTgamma.ExpandEmpty(p.ZTgamma.Size());
      this->ZTgamma = p.ZTgamma;
    }
