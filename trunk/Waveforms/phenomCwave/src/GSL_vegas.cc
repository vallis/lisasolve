...
int
GSL_vegas_integrate (gsl_monte_function * f,
                           double xl[], double xu[],
                           ...
                           double *result, double *abserr)
{
  ...
  if (state->stage <= 2)
    {
	  ...
	  std::cout << calls/2 << "   " <<pow (calls / 2.0, 1.0 / dim) << std::endl; 
	  ...
	  //exit(0);
	      ...
	      //std::cout << "box_per_bin = " << box_per_bin << \
		      "  bins =  " << bins << "  boxes = " << boxes << std::endl;
  ...
  std::cout <<  "boxes = " << state->boxes << std::endl;
  std::cout << "bins "  << state->bins << std::endl;
  ...
  for (it = 0; it < state->iterations; it++)
    {
      ...
      double *x = state->x;
      coord *bin = state->bin;
      std::cout << "jac: " << it << "   " << jacbin << std::endl; 

      state->it_num = state->it_start + it;

      std::cout << "iteration # " << it << std::endl;
      std::cout << "calls per box = " <<  calls_per_box << std::endl;
          ...
          for (k = 0; k < calls_per_box; k++)
            {
              ...
              //std::cout << "stas " << state->box[0] << "   " <<\
		      state->box[1] << "    " << state->box[2] << std::endl;
  ...
  while (j >= 0)
    {
      ...
      //std::cout << "test box " << j << "   " << box[j] << std::endl;  
  ...
  for (j = 0; j < dim; ++j)
    {
      ...
      //std::cout <<  j << "   " << box[j]  << "   " << (box[j] + gsl_rng_uniform_pos (r)) <<\
		     "   " << boxes << "   " << bins << std::endl; 
      ...
      //std::cout << "rand point  " << j << "   " << z << "   " \
	      << y << "   " << s->delx[j] << "   " << x[j] << std::endl;
      //std::cout <<  j << "   " << z << "   " \
	      << y << "   " << s->delx[j] << "   " << x[j] << std::endl;
...
resize_grid (gsl_monte_vegas_state * s, unsigned int bins)
{
  ...
  std::cout << " s-bins = " << s->bins << "  bins = " << bins << std::endl; 
  ...
  for (j = 0; j < dim; j++)
    {
      ...
      std::cout << "prob_1 " << j << "   " << grid_tot_j << std::endl;
      ...
      for (i = 0; i < bins; i++)
        {
	      ...
	      //std::cout << " prob_2 " << j << "  " << i << "   " << \
                         oldg << "   " << (oldg - 1) / oldg << "   "<<\
			weight[i] << "   " << pow(((-1. + 1./oldg) / log (1./oldg)), s->alpha)\
		       	<< std::endl; 
            }
	...
	std::cout << " pts_per_bin  = " << pts_per_bin << std::endl; 
